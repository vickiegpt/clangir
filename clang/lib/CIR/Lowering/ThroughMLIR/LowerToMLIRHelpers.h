...
mlir::Value createIntCast(mlir::ConversionPatternRewriter &rewriter,
  ...
  if (dstWidth > srcWidth && isSigned)
    return rewriter.create<mlir::LLVM::SExtOp>(loc, dstTy, src);
  else if (dstWidth > srcWidth)
    return rewriter.create<mlir::LLVM::ZExtOp>(loc, dstTy, src);
  else if (dstWidth < srcWidth)
    return rewriter.create<mlir::LLVM::TruncOp>(loc, dstTy, src);
  else
    return rewriter.create<mlir::LLVM::BitcastOp>(loc, dstTy, src);
}
